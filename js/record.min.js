    ...
    if(address){
        ...
        function getInfo(url, beforeOffset, address, limit) {
            ...
            if (address.length === 64) {
                ...
                $.ajax({
                    url: url + address + "?offset=" + offset + "&" + "limit=" + limit + sort,
                    type: "GET",
                    data: {},
                    dataType: "json",
                    async: false,
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                ...
                                if (data.balances && data.balances.length > 0) {
                                    ...
                                    layer.open({title: "请求数据失败", content: "对不起，当前数据请求失败..."})
                                }
                            },
                            error: function () {
                                layer.open({title: "请求数据失败", content: "对不起，您搜索的地址不存在..."})
                            }
                        })
                    },
                    error: function () {
        ...
        if (pageCount > 1) {
            $(".recordPages").createPage({
                pageCount: pageCount, current: 1, backFn: function (p) {
        ...
        $(".header-top-search").keydown(function () {
            if (event.keyCode == "13") {
